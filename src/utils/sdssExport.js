...
export function exportSdssPDF(result, mode = 'standard') {
  ...
  const disclaimer = 'This report is generated by the SECND SDSS v1.0 pipeline using MedGemma and PrimeKG knowledge graph verification. It is intended for informational and research purposes only and is not a confirmatory clinical diagnosis. Do not use for medical emergencies.';
...
export async function exportSdssDOCX(result, mode = 'standard') {
  ...
  // ── STORM article ──
  if (result.storm_article) {
    children.push(new Paragraph({ children: [new TextRun({ text: 'STORM Deep-Dive Article', bold: true, size: 26 })], heading: HeadingLevel.HEADING_1, spacing: { before: 200 } }));
  ...
  // ── P1 ──
  if (result.p1_differential) {
    children.push(new Paragraph({ children: [new TextRun({ text: 'MedGemma Full Reasoning (P1)', bold: true, size: 26 })], heading: HeadingLevel.HEADING_1, spacing: { before: 200 } }));
...
<tr><th>#</th><th>Diagnosis</th><th>Likelihood</th><th>KG Support</th><th>KG Score</th><th>Verified</th><th>Refuted</th></tr>\n`;
  ...
  if (result.storm_article) {
    html += `<h2>STORM Deep-Dive Article</h2><div class="section-content">${stripMarkdown(result.storm_article)}</div>\n`;
  }

  if (result.p1_differential) {
    html += `<h2>MedGemma Full Reasoning (P1)</h2><div class="section-content">${stripMarkdown(result.p1_differential)}</div>\n`;
  }
  ...
  html += `<div class="disclaimer">This report is generated by the SECND SDSS v1.0 pipeline using MedGemma and PrimeKG knowledge graph verification. It is intended for informational and research purposes only and is <strong>not</strong> a confirmatory clinical diagnosis. Do not use for medical emergencies.</div>
</body></html>`;
